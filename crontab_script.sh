...
do
	...
	_payd=$( echo "$output" | jq -r --arg z $z '.[$z | tonumber]["pago"]' )
	
	if [[ "$_payd" == "1" ]]
	then

		_domain=$( echo "$output" | jq -r --arg z $z '.[$z | tonumber]["host"]' )
		_hostname=$( echo "$output" | jq -r --arg z $z '.[$z | tonumber]["uuid"]' )
		_username=$( echo "$output" | jq -r --arg z $z '.[$z | tonumber]["email"]' )
		_user_pass=$( echo "$output" | jq -r --arg z $z '.[$z | tonumber]["senhasenha"]' )
		_root_pass=$( echo "$output" | jq -r --arg z $z '.[$z | tonumber]["senharoot"]' )
		_mysql_root_pass=$( echo "$output" | jq -r --arg z $z '.[$z | tonumber]["senhamysql"]' )
